...
export default function Explorer() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex transition-colors duration-200 w-full animate-in fade-in duration-300">
      <Sidebar />
        ...
        <main className="flex-1 p-6 md:p-8 space-y-8">
          ...
          <div className="text-center md:text-left flex flex-col md:flex-row justify-between items-center gap-4">
            ...
            <div className="text-right text-xs text-slate-400 max-w-xs">
              Contract Address: <span className="font-mono text-blue-500 font-semibold break-all text-xs">{PHARMA_CHAIN_CONTRACT_ADDRESS}</span>
            </div>
          </div>
          ...
          {/* Search Bar */}
          <form onSubmit={handleSearch} className="max-w-2xl mx-auto flex gap-2 w-full">
                      ...
                      {filteredTxs.length > 0 ? (
                        filteredTxs.map((t) => (
                          ...
                          >
                            ...
                            <td className="px-6 py-4 font-mono text-blue-500 font-medium text-xs">
                              {t.txHash.substring(0, 14)}...
                            </td>
                            <td className="px-6 py-4 font-semibold text-slate-800 dark:text-slate-100 text-xs">
                              {t.batchId}
                            </td>
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 gap-2 pt-2 border-t border-card-border/30 text-xs">
                      ...
                      <div className="flex justify-between">
                        ...
                        <span className="text-slate-800 dark:text-slate-200 font-semibold text-xs">{new Date(selectedTx.timestamp).toLocaleString()}</span>
                      </div>
                    </div>
                  </div>
                ) : (
